...
const SingleTodo = ({todo, todos, setTodos}:Props) => {
  ...
  return (
    <form className='image flex md:w-[29.5%] rounded-md p-5 mt-4 xs:w-full' onSubmit={(e) => handleEdit(e, todo.id)}>

     {
        edit ? (
          <input ref={inputRef} placeholder='Edit todo' type='text' value={editTodo} onChange={(e) => setEditTodo(e.target.value)} className='flex-1 p-[5px] border-0 text-[20px] focus:outline-none' />
        ) : (
