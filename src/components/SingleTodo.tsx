...
const SingleTodo: React.FC<Props> = ({ index, todo, todos, setTodos }: Props) => {
  ...
  return (
    <Draggable draggableId={todo.id.toString()} index={index}>
      {(provided, snapshot) => (
        <form
          className={` ${snapshot.isDragging ? "shadow-md" : ""} image flex md:w-[29.5%] rounded-md p-5 mt-4 xs:w-full`}
        ...
        >
          {edit ? (
            ...
            <span className="flex-1 p-[5px] border-0 text-[20px] focus:outline-none">
              {todo.todo}
            </span>
          )}
